...
2.  멤버의 정보를 작성하고 [등록] 버튼을 클릭합니다.
    ...
    *   비밀번호 옵션에 따라 등록된 멤버의 최초 로그인 방식이 달라집니다.
    ...
    | 비밀번호 옵션 | 설명 |
    | --- | --- |
    | 멤버에게 비밀번호 설정 메일 발송 | 등록한 멤버의 이메일 주소로 비밀번호 설정 메일 발송<br>- 멤버가 이메일을 확인한 후, 비밀번호를 직접 설정하여 로그인 |
    | 관리자가 직접 등록 | 관리자가 멤버의 초기 비밀번호 직접 설정<br>- 멤버가 관리자로부터 전달받은 초기 비밀번호로 로그인한 후, 비밀번호 변경 |
