...
| 구분    | 설명                     |
| ----- | ---------------------- |
| 서버 토큰 | 파트너의 서비스마다 하나씩 발급되는 토큰 |

* 서버 토큰 별로 호출 가능한 API Scope가 다를 수 있음 | | 파트너 코드 | 파트너를 구별하기 위한 유일값
* 파트너마다 하나의 토큰이 발급됨 | | 스페이스 테마 코드 | 스페이스 앱(봇, 바로가기)에 대한 노출 여부 설정값을 지정할 수 있는 코드 |
